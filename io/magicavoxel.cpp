...
namespace MagicaVoxel {
  ...
  bool ReadHeader(PhysFS::ifstream& file, Header& out) {
    file.read((char*)&out.id, 4);
    if (!file.good()) return false;
    file.read((char*)&out.content_size, sizeof(out.content_size));
    if (!file.good()) return false;
    file.read((char*)&out.children_size, sizeof(out.children_size));
    if (!file.good()) return false;
    out.end = (int)file.tellg() + out.content_size + out.children_size;
    return true;
  }

  const int VoxFileVersion = 150;
  ...
  void Bind() {
    ...
    Bus::On(Procedure::LoadMagicaVoxel, +[](long id, const char* path) -> void* {
      ...
      memcpy(&model->palette, default_palette, sizeof(default_palette));
      ...
      while (file.good() && (int)file.tellg() < main.end) {
        ...
        if (memcmp(&header.id, "SIZE", 4) == 0) {
          file.read((char*)&model->dimensions, sizeof(model->dimensions));
        } else if (memcmp(&header.id, "XYZI", 4) == 0) {
          unsigned int count;
          file.read((char*)&count, sizeof(count));
          if (count > 0) {
            model->voxels.resize(count);
            file.read((char*)model->voxels.data(), sizeof(glm::vec4) * count);
          }
        } else if (memcmp(&header.id, "RGBA", 4) == 0) {
          // Colors 0-254 are mapped to the palette 1-255
          file.read((char*)(model->palette + 1), sizeof(glm::vec4) * 255 );
          // the last color is not used
          glm::vec4 unused;
          file.read((char*)&unused, sizeof(glm::vec4));
        }

        file.seekg(header.end, ios::beg);
      }
